...
*/
  ...
  void WritePartialSnapshot() {
    ...
    fwrite(contextData, context_size, 1, fp_);
  ...
  void WriteSnapshot() {
    ...
    for (int j = 0; j < length; j++) {
      snapshot_data[j] = static_cast<unsigned char>(at(j)); 
      //fprintf(fp, "%u", static_cast<unsigned char>(at(j)));
...
class restoreContextFromSnapshot{
    ...
    public:
        restoreContextFromSnapshot(Local<String> snapshot_file)
        {
            /*
    ...
    Handle<v8::internal::Context> NewContextFromSnapshot() {
    if (context_size_ == 0) {
        return Handle<v8::internal::Context>();
    }
    ...
    cout << "Inside NewContextFromSnapshot"<<"\n";
    if(root->IsHeapObject())
    {
      cout << "Object is a heap object";
 ...
 */
    ...
    cout << "After check for context"<<"\n";

    //return Handle<Context>(i::Context::cast(root));
    return Handle<v8::internal::Context>(i::Context::cast(root));
...
public:
    ...
    static NAN_METHOD(New) {
        ...
        {
            cout << "\nin New method before restoring context \n";

            restoreContextFromSnapshot rCont(args[1]->ToString());
            Handle<v8::internal::Context> lcontext = rCont.NewContextFromSnapshot();
            
            cout << "\nin New method after restoring context \n";
            //NanAssignPersistent(Context, context, lcontext);
            //NanAssignPersistent(Object, proxyGlobal, lcontext->Global());
            NanReturnValue(args.This());
